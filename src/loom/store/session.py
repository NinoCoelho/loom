...
class SessionStore:
    ...
    def replace_history(self, session_id: str, messages: list[ChatMessage]) -> None:
        ...
        for seq, msg in enumerate(messages):
            ...
            self._db.execute(
                "INSERT INTO messages (session_id, seq, role, content, tool_calls, tool_call_id, name) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (session_id, seq, msg.role.value, msg.content, tc_json, msg.tool_call_id, msg.name),
            )
        self._db.execute(
    ...
    def bump_usage(self, session_id: str, input_tokens: int, output_tokens: int, tool_calls: int) -> None:
        self._db.execute(
            "UPDATE sessions SET input_tokens = input_tokens + ?, output_tokens = output_tokens + ?, tool_call_count = tool_call_count + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
    ...
    def search(self, query: str, limit: int = 20) -> list[dict[str, Any]]:
        rows = self._db.execute(
            "SELECT DISTINCT s.id, s.title, s.updated_at FROM sessions s JOIN messages m ON s.id = m.session_id WHERE m.content LIKE ? ORDER BY s.updated_at DESC LIMIT ?",
            (f"%{query}%", limit),
        ).fetchall()
