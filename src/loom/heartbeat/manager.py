...
class HeartbeatManager:
    ...
    def enable(self, args: dict) -> str:
        return self._set_enabled(args, True)

    def disable(self, args: dict) -> str:
    ...
    def list_heartbeats(self, _args: dict | None = None) -> str:
        records = self._registry.list()
        if not records:
            return "no heartbeats registered"
        lines: list[str] = []
        for r in records:
            runs = self._store.list_runs_for_heartbeat(r.id)
