...
class EchoTool(ToolHandler):
    @property
    def tool(self):
        return ToolSpec(name="echo", description="echo", parameters={"type": "object", "properties": {"text": {"type": "string"}}, "required": ["text"]})
...
class TestMemoryToolHandler:
    ...
    async def test_write_and_read(self, memory_dir, memory_index):
        ...
        r = await tool.invoke({"action": "write", "key": "test", "content": "hello", "category": "notes"})
    ...
    async def test_search(self, memory_dir, memory_index):
        ...
        await tool.invoke({"action": "write", "key": "grpc", "content": "gRPC microservices", "category": "notes"})
